...
> 이 프로젝트(PromptReflect)는 위 대회의 **Reader 성능 최적화**를 위해 개발한 프롬프트 자동 최적화 시스템입니다.
...
TextGrad의 텍스트 기반 최적화 방식과 Reflexion의 Multi-Agent 자기 개선 구조를 결합하여, 한국어 EM QA에 맞춘 프롬프트 자동 최적화 시스템을 설계했습니다. 파라미터 업데이트 없이, **언어적 피드백만으로 프롬프트를 반복 개선**합니다.
...
따라서 이 프로젝트에서도 디코더 모델을 활용하여 현실적인 RAG 환경을 반영하고자 했습니다.
...
**정리하면**: 디코더 모델은 정답이 포함된 문장(Span)을 정밀하게 찾아내지만,
...
이 프로젝트는 단일 프레임워크의 구현이 아니라, **Reflexion과 TextGrad의 하이브리드**입니다.
...
- **EM의 all-or-nothing 특성**: 단 한 글자 차이로 0점 처리
...
| 프레임워크 | 특징 | 이 프로젝트 결과 |
|------------|------|------------------|
| TextGrad | 텍스트 기반 경사하강법, 피드백 기반 개선 | Validation EM +7% 향상 확인 |
...
이 프로젝트도 같은 방식을 따르지만, **EM(Exact Match) 평가 기준**의 특수성 때문에 커스터마이징이 필요했습니다.
...
| **최적화** | Gradient Descent 흉내 | Rule-based Refinement | EM의 0/1 특성 대응 |
...
⚠️ **위 결과는 Validation Set 기준이며, Test Set에서는 성능 향상이 이어지지 않았습니다.**
...
| **EM의 all-or-nothing 특성** | 정답 기준 불일치로 인해 상충되는 피드백 누적 |

### 7.2 돌아보며

20% 샘플링, 롤백, 조기종료 등 과적합 방지 장치가 있었지만 **동일 데이터 반복 평가의 구조적 한계**를 극복하지 못했습니다. TextGrad 방식 자체의 한계라기보다, **EM의 all-or-nothing 특성 + 정답 기준 불일치 + 작은 데이터셋** 조합이 프롬프트 최적화의 일반화를 어렵게 만들었습니다.

### 7.3 Reflexion과의 연결

이 문제는 Reflexion 논문의 한계와도 맞닿아 있습니다. Reflexion은 **같은 과제를 반복**하면서 반성하는 구조라, 본 적 없는 새로운 유형의 과제에는 반성이 전이되지 않습니다. PromptReflect에서 Validation set에 특화된 프롬프트가 Test set에서 안 먹힌 것도 결국 같은 문제 — **경험을 일반화하는 것**이 이 방식의 아직 풀리지 않은 과제입니다.
